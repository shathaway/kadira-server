...
while [[ true ]]; do
  ...
  if [ -z ${START_TIME+x} ] || [ -z ${END_TIME+x} ]; then
      mongo $(pick-mongo-primary $MONGO_METRICS_URL) env-$PROFILE-$PROVIDER.js profiles/$PROFILE.js providers/$PROVIDER.js $ENV_FILE_NAME lib.js mapreduce.js incremental-aggregation.js;
  else
      mongo $(pick-mongo-primary $MONGO_METRICS_URL) env-$PROFILE-$PROVIDER.js profiles/$PROFILE.js providers/$PROVIDER.js $ENV_FILE_NAME lib.js mapreduce.js batch-aggregation.js;
  fi
  set +x;
  completedAt=$(date +%s)
  ...
  if [[ $ONLY_ONCE ]]; then
    echo "DIAGNOSTIC map-reduce mongo completing this this shell process."
    break
  fi
